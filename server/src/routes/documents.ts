...
router.post("/", async (req: AuthedRequest, res) => {
  ...
  const document = await prisma.document.create({
    data: { studentId: req.studentId!, ...parsed.data, status: "PENDING" },
  });
...
router.post("/:id/verify", async (req: AuthedRequest, res) => {
  ...
  const document = await prisma.document.findFirst({
    where: { id: documentId, studentId: req.studentId },
  });
  ...
  const updated = await prisma.document.update({
    ...
    data: {
      ...
      verifiedSource: verified ? DOCUMENT_SOURCE[document.type] ?? "Government Database" : null,
    },
  });
  ...
  if (verified) {
    await prisma.notification.create({
      data: {
        ...
        message: `Your ${document.type.replaceAll("_", " ").toLowerCase()} was successfully verified via ${DOCUMENT_SOURCE[document.type] ?? "a government database"}.`,
